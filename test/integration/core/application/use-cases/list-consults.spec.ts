...
import { HistoricConsults, ListConsults } from "@domain/use-cases/consult"
import { Account, Address, Consult } from "@domain/entities"
import { HistoricConsultsUsecase, ListConsultsUsecase } from "@application/use-cases"
...
describe('HistoricConsults', () => {
    ...
    let sut: ListConsults

    beforeAll(() => {
        connection = new ConnectionDatabase()
        repositoryFactory = new DbRepositoryFactory()
        sut = new ListConsultsUsecase(repositoryFactory)
    })
    ...
    it('should list consults by accountId', async () => {
